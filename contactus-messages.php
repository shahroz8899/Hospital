...
?>
  ...
  <script type="text/javascript">
    ...
    function del_mail() {
      ...
      if(arr<1)
      {
        warning_sweatAlert("Email Deletion", "No email has been checked to delete!!!");
      }
      else
      {
        $.ajax({
          ...
          success: function(res){
            if(res = "del"){
              ...
              error_sweatAlert("Email Deletion", "Oops! Something went wrong, Error: " + res);
            }
          },
          error: function(res){
            error_sweatAlert("Email Deletion", "error " + res);
          }
        });
      }
    }
    ...
    function refresh_contactmails(){
      $.ajax({
        type: 'POST',
        url: 'ajax_call/contactus/refresh.php',
        data: {'contactus_mails':'contactus_mails'},
        success: function(res){
          $("#contactus_mail_table").html(res);   
        },
        error: function(res){
          error_sweatAlert("Refresh Email", "Unable to refresh, Error: " + res);
        }
      });
    }  
  </script>
                ...
                <table id="contactus-mails" class="table table-hover table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="mailbox-name"><a href="read-message.php?contactmail_id=<?=$id?>"><?= $username ?></a></td>
